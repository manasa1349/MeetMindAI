...
function UploadMeeting({ onNavigate }) {
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      const meetingResponse = await meetingAPI.createMeeting({
        title: fileName ? fileName.replace(/\.[^/.]+$/, '') : 'Recorded meeting',
        date: new Date().toISOString(),
        participants: 1,
        description: 'Uploaded meeting audio'
      })

      if (!meetingResponse.success || !meetingResponse.data?._id) {
        setError(meetingResponse.message || 'Failed to create meeting before AI processing')
        return
      }

      const meetingId = meetingResponse.data._id
      const response = await meetingAPI.generateAI(meetingId, audioFile)
      
      if (response.success) {
        setSuccess('AI processing completed! Check the Transcript and Summary sections.')
        setTimeout(() => onNavigate('meeting-detail', meetingId), 900)
      } else {
